...
namespace YuefTORSetting
{
    public class SubModule : MBSubModuleBase
    {
        ...
        {
            InformationManager.DisplayMessage(new InformationMessage("TOR_Submodule Author: YuefChen", Colors.Green));
        }
        ...
        private void ApplyPatches()
        {
            ...
            {
                // 打印补丁开始信息，便于调试
                InformationManager.DisplayMessage(new InformationMessage("Applying patches...", Colors.Yellow));

                // 使用 Harmony 为模块中的所有方法应用补丁
                HarmonyInstance.PatchAll(Assembly.GetExecutingAssembly());

                // 打印补丁完成信息
                InformationManager.DisplayMessage(new InformationMessage("Patches applied successfully.", Colors.Green));
